...
from models.networks.architecture import UpBlock, DownBlock
from models.networks.architecture import ToRGB, ToRGB2
from models.networks.jpu import FlexibleJPU
from models.networks.sync_batchnorm import SynchronizedBatchNorm2d
import models.networks.standard as AlacGAN

class GodUGenerator(BaseNetwork):
    """
    メタプロ使ってU-NETベースの神クラスを作る．
    GdownのNormや深さngf等は全てconfigから指定できるようにする．
    """
    def __init__(self, opt):
        ...
        for i in range(opt.depth):
            ...
            opt_copy.semantic_nc = opt.semantic_nc + 2**i * opt.ngf
            self[f'up{i}'] = UpBlock(2**(i+1) * opt.ngf, 2**i * opt.ngf, opt_copy)

        self[f'down{0}'] = nn.Sequential(
            nn.Conv2d(opt.semantic_nc, opt.ngf, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.2, True),
        )
        for i in range(1, opt.depth+1):
            self[f'down{i}'] = DownBlock(2**(i-1) * opt.ngf, 2**i * opt.ngf, opt)

    def forward(self, input, z=None):
        opt = self.opt
        if z is not None:
            z = z.reshape(-1, self.opt.z_dim)
        latent = {0: self[f'down{0}'](input)}
        for i in range(1, opt.depth+1):
            latent[i] = self[f'down{i}'](latent[i-1])

        x, out = latent[opt.depth], None
        for i in reversed(range(opt.depth)):
            x, out = self[f'up{i}'](x, input, append=latent[i], style=z, skip=out)
        return out
